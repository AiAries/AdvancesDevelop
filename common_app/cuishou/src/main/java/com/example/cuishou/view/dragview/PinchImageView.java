package com.example.cuishou.view.dragview;
...
public class PinchImageView extends android.support.v7.widget.AppCompatImageView  {
    private final float scaledefault=-1111111;
    private float scalebase=scaledefault;
    ...
    protected float calculateNextScale(float innerScale, float outerScale) {
        ...
        if (currentScale < MAX_SCALE/2) {
            return MAX_SCALE/2;
        }else if (currentScale < MAX_SCALE&&currentScale >= MAX_SCALE/2) {
            return MAX_SCALE;
        } else if (outerScale==MAX_SCALE) {
            return innerScale;
        } else {
            return innerScale;
        }
    ...
    private GestureDetector mGestureDetector = new GestureDetector(PinchImageView.this.getContext(), new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            if (mPinchMode == PINCH_MODE_FREE && !(mScaleAnimator != null && mScaleAnimator.isRunning())) {
                float innerScale = MathUtils.getMatrixScale(getInnerMatrix(MathUtils.matrixTake()))[0];
                if(isSingleFinger&&scalebase<=innerScale){}
                else{fling(velocityX, velocityY);
                }
            }
            return true;
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if(action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) {
            float innerScale = MathUtils.getMatrixScale(getInnerMatrix(MathUtils.matrixTake()))[0];
            if(action == MotionEvent.ACTION_UP&&mDragViewLayout!=null&&scalebase<=innerScale)mDragViewLayout.onTouchEvent(event);
            ...
            isSingleFinger=true;
            float innerScale = MathUtils.getMatrixScale(getInnerMatrix(MathUtils.matrixTake()))[0];
            if(scalebase==scaledefault){
                scalebase=innerScale;
            }
            if(mDragViewLayout!=null&&scalebase<=innerScale)mDragViewLayout.onTouchEvent(event);
            ...
            if(isSingleFinger){
                float innerScale = MathUtils.getMatrixScale(getInnerMatrix(MathUtils.matrixTake()))[0];
                if(mDragViewLayout!=null&&scalebase<=innerScale)mDragViewLayout.onTouchEvent(event);
                if(scalebase<=innerScale)return true;//如果只要一个手指头，且没有缩放，则直接不操作下去
            }
            if (!(mScaleAnimator != null && mScaleAnimator.isRunning())) {
                //在滚动模式下移动
